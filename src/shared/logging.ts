...
export function friendlyDiscordError(error: unknown, action: string): string {
  ...
  if (code === 50001) {
    return `${prefix}: Discord reports that Bot Dashboard is missing access to that channel.`;
  }
  if (code === 50013) {
    return `${prefix}: Bot Dashboard is missing one or more required channel permissions. Check View Channel, Send Messages, Embed Links, and Attach Files.`;
  }
  if (code === 10003) {
