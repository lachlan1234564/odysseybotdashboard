...
export async function handleServerInfo(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  const embed = new EmbedBuilder()
    ...
    .setTitle(guild.name)
    .setDescription("A live overview of this Discord server and its Bot Dashboard setup.")
    .addFields(
      {
        name: "Owner",
        value: `${ownerName}\n<@${guild.ownerId}>`,
    ...
    .setFooter({
      text: branding.footerText || "Bot Dashboard • Server overview",
...
export async function handleUserInfo(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  const embed = new EmbedBuilder()
    ...
    .setFooter({ text: "Bot Dashboard user info" });
...
export async function handleSocialsPost(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  if (missingPermissions.length > 0) {
    await replyToCommand(interaction, {
      content: "Bot Dashboard cannot post there. Check View Channel, Send Messages, and Embed Links permissions."
    });
    return;
  }
